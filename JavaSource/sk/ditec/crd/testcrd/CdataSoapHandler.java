...
/**
 * Sluzi na manipulaciu s odchadzajucou spravou Zmeny su testovane pre JAVA 8 u202 a Tomcat 6 Implemenovane: - ak
 * element obsahuje text <![CDATA[ tak je modifikovany aby obsahoval CDATASection sekciu, tym sa efektivne zabrani
 * escapovaniu CDATA pri finalnej serializacii
 */
public class CdataSoapHandler implements SOAPHandler<SOAPMessageContext> {
	private Logger log = LoggerFactory.getLogger(CdataSoapHandler.class);

	@Override
	public boolean handleMessage(SOAPMessageContext messageContext) {
		try {
			Boolean outboundProperty = (Boolean) messageContext.get(MessageContext.MESSAGE_OUTBOUND_PROPERTY);
			if (outboundProperty) { // ak je odchadzajuca sprava, manipuju s nou a nastav kazdy element s CDATA ako
									// CDATASekciu, ak uz taku nema
		...
		if (node.getNodeType() == Node.ELEMENT_NODE) {
			...
			for (int i = 0; i < childNodes.getLength(); i++) {
				...
				if (child.getNodeType() == Node.TEXT_NODE && child.getNodeValue().startsWith(_CudConsts.CDATA_START)
					...
					log.info("prebehla manilupacia s obsahom, bola nastavena ako CDATA sekcia pre "
							+ child.getLocalName());
