...
public class CudExportCrdProcess extends BaseProcess {

	private Logger log = LoggerFactory.getLogger(CudExportCrdProcess.class);
			...
			for (DTOObjekt dtoobj : listObj) {
				...
				for (DTOOdberatelObjekt dtooo : listoo) {
					...
					if (dtooo.getCasPoslExportu() == null) {
						...
						dtooo.setCasPoslExportu(new SimpleDateFormat("dd/MM/yyyy").parse("01/01/2025"));
					}

					// pre ucely ladenia
					// dtooo.setCasPoslExportu(new SimpleDateFormat("dd.MM.yyyy HH:mm").parse("28.01.2026 08:01"));
	...
	public ActionResult exportCrdDat(AuthInfo auth, DTOOdberatelObjekt dtooo, Date datumAcasNacitania)
		...
		DTOTCompany tCompanySk = dlgCrd.getTCudCiselnikyClass().getCompany(auth, _CudConsts.COMPANY_UIC_CODE_ZSR);
		...
		try {
			...
			DTOWfDef dtoWfPl = dlg.getWfDefRead().readLastLight(auth, dtoCisPrimLoc.getCiselnikID(),
					_CudConsts.WF_DEF_TYP_ES);
			DTOWfDef dtoWfSl = dlg.getWfDefRead().readLastLight(auth, dtoCisSubLoc.getCiselnikID(),
					_CudConsts.WF_DEF_TYP_ES);

			// 4 / idCountrySk countryKodIso = "SVK"
			DTOTCountry tCountry = dlgCrd.getTCudCiselnikyClass().getCountryByIso(auth, _CudConsts.COUNTRY_CODE_ISO_SK);
			...
			// aa datumAcasPoslSprac = new SimpleDateFormat("dd.MM.yyyy HH:mm").parse("05.09.2025 15:01");
			// 6. /*$PrimaryList
			ArrayList<DTOTPrimaryLocation> listPrimaryLocation = dlgCrd.getTCudCiselnikyClass()
					.getPrimaryLocationListForExport(auth, datumAcasNacitania, datumAcasPoslSprac,
							tCountry.getCountryID(), tCompanySk.getCompanyID());
			//
			// 6. /*$PrimaryListBuduce*/
			// vyh¾adám všeky záznamy, ktoré vznikli medzi posledným exportom a aktuálnym dátumom
			...
			for (DTOTPrimaryLocation dtopl : listPrimaryLocation) {
				...
				DTOTPrimaryLocation dtoplPredch = dlgCrd.getTCudCiselnikyClass().getPrimaryLocationListPredch(auth,
						dtopl.getPrimaryLocationID(), dtopl.getIDCountry(), datumAcasPoslSprac, datumAcasNacitania);
				...
				int messageStatus;
				if (dtoplPredch != null && "T".equals(dtoplPredch.getZmaz()) && "T".equals(dtopl.getZmaz())) {
					// bol vytvoreny aj zrudeny zaznam
					continue;
				}
				...
				//("F".equals(dtopl.getEndValidity() == null
				//		|| dtopl.getEndValidity().getTime() >= dtopl.getStartValidity().getTime())) {
					// //Ak EndValidty je null alebo EndValidity>=StartValidity
				...
				ms.setValue(_CudConsts.COMPANY_UIC_CODE_ZSR);
				mh.setSender(ms);
				...
				country.setValue(_CudConsts.COUNTRY_CODE_ISO_SK);
				...
				lpi.setResponsibleIM(_CudConsts.COMPANY_UIC_CODE_ZSR);
				lpi.setLocationPrimaryName(dtopl.getLocationName());
				lpi.setPrimaryLocationNameASCII(dtopl.getLocationNameAscii());
				lpi.setNUTSCode(dtopl.getNutsCode());
				lpi.setContainerHandlingFlag("T".equals(dtopl.getContainerHandlingFlag()));
				lpi.setHandoverPointFlag("T".equals(dtopl.getHandoverPointFlag()));
				lpi.setComments(dtopl.getFreeText());
				...
				if (dtopl.getFreightStartValidity() != null || dtopl.getFreightEndValidity() != null) {
					...
					freightFlagValidityPeriod.setStartDate(CudVysielanieUtils.dateToXmlDate(dtopl
							.getFreightStartValidity()));
					freightFlagValidityPeriod
							.setEndDate(CudVysielanieUtils.dateToXmlDate(dtopl
				...
				if (dtopl.getPassengerStartValidity() != null || dtopl.getPassengerEndValidity() != null) {
					passengerValidityPeriod.setStartDate(CudVysielanieUtils.dateToXmlDate(dtopl
				...
				if (dtopl.getLatitude() != null) {
					coordinates.setLatitude(dtopl.getLatitude());
					// float moze byt na 6 miest, jedine, ked je 64bitova JAva
				}
				if (dtopl.getLongitude() != null) {
					coordinates.setLongitude(dtopl.getLongitude());
				}
				...
				ValidityPeriod v = new ValidityPeriod();
					...
					v.setStartDate(CudVysielanieUtils.dateToXmlDate(dtopl.getStartValidity()));
					v.setEndDate(CudVysielanieUtils.dateToXmlDate(dtopl.getEndValidity()));
				...
				// errorMessage="Primárnu Lokalitu treba vymaza vCRD manuálne:
				// $PrimLok.LOCATION_CODE, $PrimLok.START_VALIDITY
				// NavratovyKod = 1
				// NavratovyText = "CRD manuálny výmaz"
				// ChybovaSprava =errorMessage
				// XML=null
				if (messageStatus == 3) // delete
				{

					String emailText = "<br/><b> Primárnu Lokalitu je potrebné vymazať v CRD manuálne: </b>"
							+ "<br/> PrimLok.Code: " + dtopl.getLocationCode() + "<br/> AllocationCompany: "
							+ _CudConsts.COMPANY_UIC_CODE_ZSR + "<br/> PrimLok.LocationName: "
							+ dtopl.getLocationName() + "<br/> $SubsLok.START_VALIDITY: "
							+ CudVysielanieUtils.dateToXmlDate(dtopl.getStartValidity());
					HlpOdosliSpravu.sendErrorMail(emailText, "CRD manuálny výmaz", SkupinaPrijemcov.CRD);
					dtoSendSubor.setNavratKod("1");
					dtoSendSubor.setNavratText("CRD manuálny výmaz");
					dtoSendSubor.setErrorSprava("Primárnu Lokalitu treba vymazať v CRD manuálne");
				...
				try {
					...
					// dataSend.setMessage(vratSpravuXMLForWs(mess)); // vratSpravuXMLCData(mess).toString());
					// dataSend.setMessage(new CDATA(xmlReceiptConfirmationMessage));
					// dataSend.setMessage(new CDATA(xmlReceiptConfirmationMessage));
			...
			for (DTOTSubsidiaryLocation dtosl : listSubsidiaryLocation) {
				...
				DTOTSubsidiaryLocation dtoplPredch = dlgCrd.getTCudCiselnikyClass().getSubLocationListPredch(auth,
						dtosl.getSubsidiaryLocationID(), dtosl.getIDCountry(), datumAcasPoslSprac, datumAcasNacitania);

				LocationFileDatasetMessage mess = new LocationFileDatasetMessage();
				// 8.2.1 AK NEnašlo záznam Alebo
				// AK NE našlo záznam alebo naslo a PredchadzajucaPlatnaLokalita.ZMAZ=true
				int messageStatus;
				if (dtoplPredch != null && "T".equals(dtoplPredch.getZmaz()) && "T".equals(dtosl.getZmaz())) {
					// bol vytvoreny aj zrudeny zaznam
					continue;
				}
				if (dtoplPredch == null || "T".equals(dtoplPredch.getZmaz())) {
					// $MessageStatus = 1 (NEW)
					messageStatus = 1;
					// } else if ("F".equals(dtosl.getEndValidity() == null
					// || dtosl.getEndValidity().getTime() >= dtosl.getStartValidity().getTime())) {
				...
				lsi.setAllocationCompany(_CudConsts.COMPANY_UIC_CODE_ZSR);
				lsi.setComments(dtosl.getFreeText());
				...
				v.setStartDate(CudVysielanieUtils.dateToXmlDate(dtosl.getStartValidity()));
				v.setEndDate(CudVysielanieUtils.dateToXmlDate(dtosl.getEndValidity()));

				lsi.setLocationValidityPeriod(v);
				mess.setLocationSubsidiaryInformation(lsi);

				GeographicCoordinates coordinates = new GeographicCoordinates();

				if (dtosl.getLatitude() != null) {
					coordinates.setLatitude(dtosl.getLatitude());
				}
				if (dtosl.getLongitude() != null) {
					coordinates.setLongitude(dtosl.getLongitude());
				}

				if (coordinates.getLatitude() != null && coordinates.getLongitude() != null) {
					lsi.setGeographicCoordinates(coordinates);
				}
				...
				if (messageStatus == 3) {// delete
					// /errorMessage=""Subsidiárnu Lokalitu treba vymaza vCRD manuálne:
					// Prim=$NadradenaLokalita.LOCATION_CODE,
					// AllocationCompany:$SubsLok.T_COMPANY.COMPANY_UIC_CODE,Subsidiary:
					// Subdsidiary Lock:
					// $SubsLok.SUBSIDIARY_LOCATION_CODE,
					// $SubsLok.T_SUBSIDIARY_TYPE.SUBSIDIARY_TYPE_NAME,
					// $SubsLok.START_VALIDITY

					String emailText = "<br/><b> Subsidiárnu Lokalitu je potrebné vymazať v CRD manuálne: </b>"
							+ "<br/> Prim =" + tPrimLoc.getLocationCode() + "<br/> AllocationCompany: "
							+ _CudConsts.COMPANY_UIC_CODE_ZSR + "<br/> Subdsidiary Lockation: "
							+ "<br/> $SubsLok.SUBSIDIARY_LOCATION_CODE: " + dtosl.getSubsidiaryLocationCode()
							+ "<br/> $SubsLok.T_SUBSIDIARY_TYPE.SUBSIDIARY_TYPE_NAME: "
							+ dtosl.getSubsidiaryLocationName() + "<br/> $SubsLok.START_VALIDITY: "
							+ CudVysielanieUtils.dateToXmlDate(dtosl.getStartValidity());
					HlpOdosliSpravu.sendErrorMail(emailText, "CRD manuálny výmaz", SkupinaPrijemcov.CRD);

					dtoSendSubor.setNavratKod("1");
					dtoSendSubor.setNavratText("CRD manuálny výmaz");
					dtoSendSubor.setErrorSprava("Subsidiárnu Lokalitu treba vymazať v CRD manuálne");
					dtoSendSubor.setErrorCas(new Date());
					// dtoSendSubor.setOdpovedTyp(res);
					// dtoSendSubor.setOdpovedSubor(replicationResponsePrimLoc.);
					dlgCrd.getCudSendSuborClass().update(auth, dtoSendSubor);
	...
	public String vratSpravuXML(LocationFileDatasetMessage mess) throws AppException {
		...
		try {
			...
			// Vypnutie XML deklarácie
			// m.setProperty(Marshaller.JAXB_FRAGMENT, Boolean.TRUE);

			m.marshal(mess, sw);

			String message = _CudConsts.CDATA_START +
	...
	public String vratSpravuXML(SendOutboundMessage mess) throws AppException {
	...
	public SendOutboundMessageResponse getWsCrdSend(SendOutboundMessage data, DTOOdberatelObjekt dtooo)
		...
		// log.info("CudExportCrdProcess WSSend:" + vratSpravuXML(data));
		try {

			OutboundConnectorService_Service stub = dlgCrd.getCrdExportWSFromOdberatelExportCesta(dtooo
					.getExportCesta());
			// OutboundConnectorService_Service stubCdata = new OutboundConnectorService_Service(url, qname);
			OutboundConnectorService port = stub.getOutboundConnectorServicePort();

			// pripojenie handlera

			WSBindingProvider bindingProvider = (WSBindingProvider) port;
			bindingProvider.setAddress(dtooo.getExportCesta());
			...
			SendOutboundMessageResponse res = port.sendOutboundMessage(data, true);
			
		return res;
		} catch (Throwable e) {
			handleException(e, "getWsCrdSend.error");
			return null;
		}
	}

	// byte[] tcrBinary = readBinary(
	// auth.T,
	// VylCudSendSuborPeer.TABLE_NAME,
	// "SUBOR",
	// VylCudSendSuborPeer.SEND_SUBOR_ID,
	// subor.getSendSuborId());
	// 30/10/2025 12:30 • SendOutboundMessage sendOutboundMessage = new SendOutboundMessage();
	// sendOutboundMessage.setMessage(
