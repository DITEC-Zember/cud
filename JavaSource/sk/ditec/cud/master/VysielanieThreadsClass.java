...
public class VysielanieThreadsClass extends _CudBaseClass {

	static Logger log = LoggerFactory.getLogger(VysielanieThreadsClass.class);
	...
	protected static void postDataToUrl(String targetURL, String login, String pass, byte[] data) throws Exception {
		...
		try {
			...
			// log.info("Vlakno {} poslalo odpoved responseCode={}", logID, connection.getResponseCode());
	...
	public static void postData(final List<DTOSubor> dataList) throws InterruptedException {

		// Pomôcka na synchronizáciu štartu všetkých vlákien
		...
		for (DTOSubor dto : dataList) {
			...
			executor.submit(new Runnable() {
				public void run() {
					String id = null;
					try {
						// Vlákno čaká, kým hlavné vlákno neodpočíta latch
						latch.await();

						id = StringUtils.replaceAll("logID={1}, index=({2})", "{1}", dtoItem.getSuborID().toString());
						id = StringUtils.replaceAll(id, "{2}", index + "/" + dataList.size());

						log.info("Posielanie spravy {}", id);

						postDataToUrl("http://kiptest.intra.zsr.sk/comm_pis/InboundMessage?wsdl", "PIS_COMM", "PIS_COMM", dtoItem.getSubor());

					} catch (Exception e) {
						log.error("Chyba vo vlakne {}", id);
					}
				}
			});
		}

		System.out.println("START! Posielam requesty...");
		latch.countDown(); // "Výstrel z pištole" - všetky vlákna sa pohnú naraz
		...
		try {
			...
			while (true) {
				...
				if (dataList.isEmpty()) {
					...
					postData(dataList);
					minLogID = dataList.get(dataList.size() - 1).getSuborID();
				}
